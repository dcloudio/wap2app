...
var mui = (function(document, undefined) {
    ...
    $.later = function(fn, when, context, data) {
        ...
        f = function() {
            m.apply(context, $.isArrayLike(d) ? d : [d]);
        };
...
(function($, window) {
    function detect(ua) {
        ...
        var funcs = [
            function() { //qqbrowser
                var qqbrowser = ua.match(/(QQBrowser)\/([\d\.]+)/);
                if (qqbrowser) { //qqbrowser
                    this.os.qqbrowser = {
                        version: qqbrowser[2].replace(/_/g, '.')
                    };
                }
                return false;
            },
            function() { //ucbrowser
                var ucbrowser = ua.match(/(UCBrowser)\/([\d\.]+)/);
                if (ucbrowser) { //ucbrowser
                    this.os.ucbrowser = {
                        version: ucbrowser[2].replace(/_/g, '.')
                    };
                }
                return false;
            },
            function() { //qihoobrowser
                if (/qihoobrowser/i.test(ua)) { //qihoobrowser
                    this.os.qihoobrowser = true;
                }
                return false;
            },
            function() { //wechat
    ...
    var delegateFn = function(element, event, selector, callback) {
        return function(e) {
            ...
            for (; target && target !== document; target = target.parentNode) {
                ...
                if (~['click', 'tap', 'doubletap', 'longtap', 'hold'].indexOf(event) && (target.disabled || (target.classList && target.classList.contains('mui-disabled')))) {
                    break;
                }
                var matches = {};
    ...
    var handle = function(event, touch) {
        ...
        switch (event.type) {
            case $.EVENT_END:
                ...
                var findDisabledTarget = target;
                var isReturn = false;
                for (; findDisabledTarget && findDisabledTarget !== document; findDisabledTarget = findDisabledTarget.parentNode) {
                    if (findDisabledTarget.disabled || (findDisabledTarget.classList && findDisabledTarget.classList.contains('mui-disabled'))) {
                        isReturn = true;
                    }
                }
                if (isReturn) {
...
(function($, window, name) {
	...
	$.fn['switch'] = function(options) {
		...
		this.each(function() {
			...
			if (!id) {
				id = ++$.uuid;
				switchApi = $.data[id] = new Toggle(this);
...
(function($, document) {
    ...
    var SELECTOR_TAB_ITEM = '.' + CLASS_TAB_ITEM;

    var IFRAME_STYLES = 'display:{DISPLAY};position: fixed;left: 0;top: {TOP}px;right: 0;bottom: {BOTTOM}px;width:100%;height:{HEIGHT}px;z-index: 1;border:0;';

    var Tab = function(element, options) {
        this.element = element;
        this.options = $.extend(true, {
            type: $.os.plus ? 'webview' : 'iframe',
            index: 0,
            styles: {
                top: undefined,
                bottom: undefined,
        ...
        if (typeof styles.top === 'undefined') {
            ...
            if (headerElem) {
                styles.top = headerElem.offsetHeight + 1;
            } else {
                styles.top = 0;
            }
        }
        if (typeof styles.bottom === 'undefined') {
            styles.bottom = element.offsetHeight + 1;
        }

        this.itemElems = $.slice.call(element.querySelectorAll(SELECTOR_TAB_ITEM));
        this.ids = [];
        this.shown = [];
    ...
    proto.init = function() {
        if (this.itemElems && this.itemElems.length) {
            if ($.os.plus && this.options.type === 'webview') {
                $.plusReady(function() {
    ...
    proto.initEvent = function() {
        this.element.addEventListener('tap', function(e) {
            var target = e.target;
            for (; target && target !== document; target = target.parentNode) {
                if (target.__mui_tab_id) {
                    this.show(target.__mui_tab_id, false);
                    break;
                }
            }
        }.bind(this));
    };
    proto._getId = function(itemElem, url) {
        var id = itemElem.getAttribute('data-tab-id') || url;
        this.ids.push(id);
        itemElem.__mui_tab_id = id;
        return id;
    };
    proto._initTabByWebview = function() {
        var currentWebview = plus.webview.currentWebview();
        for (var i = 0, len = this.itemElems.length; i < len; i++) {
            var itemElem = this.itemElems[i];
            var url = itemElem.href;
            var id = this._getId(itemElem, url);
            var webview = plus.webview.create(url, id, this.options.styles);
            webview.hide('none');
            currentWebview.append(webview);
        }
        this.type = 'webview';
    };
    proto._initTabByIframe = function() {
        var styles = this.options.styles;
        var top = parseFloat(styles.top);
        var bottom = parseFloat(styles.bottom);
        var height = parseFloat(window.innerHeight - top - bottom);
        for (var i = 0, len = this.itemElems.length; i < len; i++) {
            var itemElem = this.itemElems[i];
            var url = itemElem.href;
            var id = this._getId(itemElem, url);
            ...
            if (!iframe) {
                ...
                iframe.id = id;
                iframe.src = url;
                iframe.setAttribute('style', IFRAME_STYLES.replace('{DISPLAY}', 'none').replace('{TOP}', top).replace('{BOTTOM}', bottom).replace('{HEIGHT}', height));
                document.body.appendChild(iframe);
            }
        }
        this.type = 'iframe';
    };
    proto.show = function(id, active) {
        if (typeof id === 'number') {
            id = this.ids[id];
        }
        ...
        if (~index) {
            ...
            if (active !== false) {
                var lastItemElem = this.element.querySelector('.' + CLASS_ACTIVE);
                lastItemElem && lastItemElem.classList.remove(CLASS_ACTIVE);
                var currentItemElem = this.itemElems[index];
                currentItemElem.classList.add(CLASS_ACTIVE);
            }
            if (this.type === 'webview') {
                if (~this.shown.indexOf(id)) {
                    plus.webview.show(id);
                } else {
                    plus.webview.show(id, this.options.show.aniShow, this.options.show.duration);
                    this.shown.push(id);
                }
    ...
    proto.hide = function(id) {
        if (typeof id === 'number') {
            id = this.ids[id];
        }
        if (this.type === 'webview') {
            plus.webview.hide(id, 'auto');
        } else if (this.type === 'iframe') {
            var iframeElem = document.getElementById(id);
    ...
    $.fn.tab = function(options) {
        if (this.length === 1) {
            var self = this[0];
            var tabApi = self.mui_plugin_tab;
            options = options || {};
            if (!tabApi) {
                tabApi = self.mui_plugin_tab = new Tab(self, options);
            }
            return tabApi;
        }
    };
